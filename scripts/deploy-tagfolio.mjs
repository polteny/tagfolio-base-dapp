...
const source = await readFile(resolve(process.cwd(), "contracts/Tagfolio.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "Tagfolio.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["Tagfolio.sol"]?.Tagfolio;
...
console.log(`Deploying Tagfolio from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TAGFOLIO_CONTRACT_ADDRESS to this contract address in Vercel.");
